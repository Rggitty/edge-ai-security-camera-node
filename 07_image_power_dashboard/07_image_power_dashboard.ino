...
// =======================
// Save latest captured JPEG
// =======================
bool saveLatestImage(camera_fb_t *fb) {
  if (!fb || fb->len == 0) {
    return false;
  }
...
if (saved) {
  lastState = "capture_ok_image_saved";
} else {
  lastState = "capture_ok_image_save_failed";
}
}
...
String htmlPage() {
  ...
  page += "img{max-width:100%;border-radius:12px;border:2px solid #444;margin-top:10px;}";
  page += "</style>";
  ...
  page += "if(d.image_bytes > 0 && d.image_version != lastVersion){";
  page += "lastVersion = d.image_version;";
  page += "document.getElementById('latestimg').src='/latest.jpg?t='+Date.now();";
  page += "document.getElementById('imgstatus').innerText='Latest captured image loaded';";
  page += "}";

  page += "}";

  page += "async function resetStats(){";
  page += "await fetch('/reset');";
  page += "update();";
  page += "}";

  page += "async function forceCapture(){";
  page += "await fetch('/capture');";
  page += "setTimeout(update,500);";
  page += "}";

  page += "async function clearLog(){";
  page += "await fetch('/clearlog');";
  page += "update();";
  page += "}";
  ...
  page += "<div class='card'>";
  page += "<div>State</div>";
  page += "<div class='big good' id='state'>...</div>";
  page += "</div>";

  page += "<div class='card'>";
  page += "<h2>Latest Motion Capture</h2>";
  page += "<p id='imgstatus'>No image captured yet. Move in front of the PIR sensor or press Force Capture.</p>";
  page += "<img id='latestimg' alt='Latest captured frame will appear here'>";
  page += "</div>";
  ...
  page += "<tr><td>Last Event Energy, 3 sec window</td><td id='eventenergy'>...</td></tr>";
  page += "</table>";
  page += "</div>";
  ...
  page += "<button onclick='forceCapture()'>Force Capture</button>";
  page += "<a href='/eventlog.csv'><button>Download CSV</button></a>";
  page += "<button onclick='clearLog()'>Clear Event Log</button>";
  page += "<p class='warn'>Move in front of the PIR sensor to trigger camera capture and event energy tracking.</p>";
  page += "</body></html>";
...
void handleLatestImage() {
  if (latestImage == nullptr || latestImageLen == 0) {
    server.send(404, "text/plain", "No image captured yet.");
    return;
  }

  server.sendHeader("Cache-Control", "no-store, no-cache, must-revalidate, max-age=0");
  server.setContentLength(latestImageLen);
...
void handleEventCSV() {
  String csv = "event,time_ms,image_bytes,capture_us,event_energy_mWh,peak_current_mA,peak_power_mW,avg_power_mW,min_voltage_V\n";
  ...
  server.sendHeader("Content-Disposition", "attachment; filename=event_power_log.csv");
...
void handleForceCapture() {
  captureImageEvent();
  server.send(200, "text/plain", "forced capture complete");
}
...
void setup() {
  ...
  Serial.println();
  Serial.println("==== EDGE CAMERA IMAGE + POWER DASHBOARD ====");
